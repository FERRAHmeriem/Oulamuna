...
function ArticleCard({ article }) {
  return (
    <div className="bg-[#274a43fd] rounded-lg p-2">
      <div className="h-full border-2 border-stone-800 rounded-lg p-4 shadow-lg shadow-stone-700 bg-white flex flex-col justify-between">
        
        <div className="border-b-2 border-stone-800 pb-4">
          <h1 className="text-2xl text-center font-semibold text-stone-800">{article.scholarName}</h1>
        </div>
        ...
        <div className="py-4 flex-1 flex flex-col">
          ...
          <div className="flex items-start mt-4 mb-4 gap-4">
            <p className="basis-3/5">{article.description}</p>
            <img className="basis-2/5 h-52 w-40 rounded-xl object-cover shadow-stone-700 shadow-xl" src={article.imageArticle.url} alt="" />
          </div>
          <Link
        ...
        <p className="border-t-2 border-stone-800 pt-2 mt-auto text-sm" style={{ marginBottom: '10px' }}>
          Écrit par <span className='font-semibold underline cursor-pointer hover:text-emerald-800'>{article.author}</span>
        </p>
      </div>
    </div>
  );
